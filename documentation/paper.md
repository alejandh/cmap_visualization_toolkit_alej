...
graph TD
    ...
    E --> E2[Intermediate Tools]
    E2 --> E2a[Configuration]
    E2 --> E2b[Word-Based Heatmap]
    E2 --> E2c[t-SNE Plot]
    E2 --> E2d[Semantic Network]
    E2 --> E2e[Custom Groupings]
    E2 --> E2f[Code-Based Heatmap]
    ...
    E --> E3[Advanced Tools]
    E3 --> E3a[Configuration]
    E3 --> E3b[Full Visuals]
    E3 --> E3c[Interactive Version]
    
    style SPACER1 fill:none,stroke:none
    style SPACER1b fill:none,stroke:none
    style SPACER1c fill:none,stroke:none
    style SPACER1d fill:none,stroke:none
    style SPACER2 fill:none,stroke:none
    style SPACER2b fill:none,stroke:none
    style SPACER2c fill:none,stroke:none
    style SPACER2d fill:none,stroke:none
    style SPACER2e fill:none,stroke:none
```
*Figure 1. Program Organization of the CMAP Toolkit.*
![Examples of annotated blocks: (A) Package imports, (B) Validator, (C) Helper functions](figures/package_load.png)
![ ](figures/helper_functions.png)
![ ](figures/validator.png)  
*Figure 2. Examples of annotated blocks from the CMAP toolkit: (A) package imports, (B) validator, and (C) helper functions.*  
# Functions 
...
Finds words used in conceptually similar ways using dynamic contextual embeddings [@liu2019roberta]. Best for uncovering analogies and latent meanings (e.g., success → money, happiness, family). The embedding model can be changed to use fine-tuned or specialized models. 
- **Co-occurrence (Jaccard or Cosine Similarity Distance)**
  Best for identifying direct vocabulary associations in the same text segments (e.g., success → hard work, effort).
...
Highlights words that co-occur more often than expected by chance. Best for finding statistically significant pairings.
- **TF–IDF (Term Frequency–Inverse Document Frequency)** 
Detects distinctive words that are unusually important in a given segment [@manning2008introduction; @newman2010networks].
...
- **Word Clouds** – Highlight the most frequent and salient terms across a dataset or within filtered subsets, and allow color coding by theme.
- **t-SNE Semantic Maps** – Reduce high-dimensional similarity matrices into 2D plots, emphasizing seed words for interpretability.
- **Word Heatmaps & Semantic Networks** – Show how concepts or "codes" (meta-data used to index text, like #morality_talk) relate to each other on a color coded table with options for clustering.
...
The examples shown below use qualitative interview data, but CMAP can be applied to any properly formatted .csv dataset.
<table>
<tr>
<td><img src="figures/word_cloud.png" alt="Word Cloud" width="400"/></td>
<td><img src="figures/heatmap.png" alt="Heatmap" width="400"/></td>
</tr>
<tr>
<td><img src="figures/tsne.png" alt="t-SNE Plot" width="400"/></td>
<td><img src="figures/semantic_network.png" alt="Semantic Network" width="400"/></td>
</tr>
</table>
*Figure 3. Collection of Sample Visualizations: (A) Word Cloud, (B) Heatmap, (C) t-SNE Plot, (D) Semantic Network.*  
Any data can be used as long as it includes the required fields from the schema below.  

![Parameter schema for CMAP text segments](figures/schema.png)
*Table 1: Parameter schema for CMAP text segments*  

![Example in .csv, using data described in @abramson2015beyond](figures/example_csv.png)
*Figure 4: Example in .csv, using data described in Abramson and Dohan 2015*  

All parameters are configurable in labeled execution blocks, which set how the visuals are produced. For instance short text windows and few words for syntactic analyses, larger windows and more seeds to look at overlapping themes. Users can designate colored grouping to correspond to deeper readings of text [@abramson2024inequality].  
![Configurable Execution Block](figures/config.png)
*Figure 5: Configurable Execution Block*  
